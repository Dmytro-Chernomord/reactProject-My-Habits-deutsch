...
const Swipers = () => {
  const payment = useSelector(selector.getCreditCard);
  const newArr = payment.map(el => (
    <SwiperSlide key={el.number} tag="li">
      <div className={style.card}>
        <img src={visa} alt="visa" className={style.svgStyle} />

        <span className={style.cardNumb}>{el.number}</span>
        <div className={style.cardDelete}>
          <span className={style.cardDate}>
            gültig bis: {el.timeExpiration}
          </span>
