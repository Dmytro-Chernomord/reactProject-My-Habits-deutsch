...
const AddCreditCardForm = ({ closeForm }) => {
  ...
  const onSubmit = e => {
    ...
    if (monthValue > 12) {
      setDateValid('*nur 12 Monate im Jahr');
      return;
    } else if (yearValue < year && dateValue.length !== 0) {
      setDateValid('*Die Karte ungültig');
      return;
    ...
    <div className={styles.boxFormPassword}>
      ...
      <form className={styles.formProfile} onSubmit={onSubmit}>
        <div className={styles.boxErrorCard}>
          {errorNumber && <p className={styles.error}>*falsche Kartenummer</p>}
          {errorRequiredNumber && <p className={styles.error}>*erforderlich</p>}
        </div>
        <label className={styles.label}>
          <span className={styles.textLabelCard}>Kartenummer</span>
          <CardNumbInput
        ...
        <div className={styles.boxErrorCard}>
          {errorDate && <p className={styles.error}>*falsches Datum</p>}
          {errorRequiredDate && <p className={styles.error}>*erforderlich</p>}
        ...
        <div className={styles.boxErrorCard}>
          {errorShow && <p className={styles.error}>*erforderlich</p>}
          <div style={{ marginBottom: '5px' }}>
            <NotificationModal />
          </div>
        </div>
        <Button variety={'white'} text="Hinzufügen" />
      </form>
    </div>
  );
};
